...
from .speech.compare import compare
...
def _warm_up() -> None:
    def run():
        try:
            tts.load()
            asr.warm_up()
            log.info("Models ready (TTS: %s, ASR: %s)", tts.engine, config.WHISPER_MODEL)
        except Exception:
...
async def attempt(target: str = Form(..., max_length=500), audio: UploadFile = File(...)):
    ...
    if pcm.size < 16000 * 0.2 or peak_level(pcm) < 0.02:
        ...
        heard = asr.transcribe(trim_and_pad(pcm))
    return {"heard": heard, **compare(target, heard)}
